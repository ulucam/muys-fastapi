...
def _operator_baglantilari(db: Session, rol: str, istasyon_id: int | None, personel_id: int | None, haric_id: int | None = None):
    if rol != "Operatör":
    ...
    return istasyon, personel, sorgu.first() if sorgu is not None else None
...
def kullanici_olustur(db: Session, **veri) -> KullaniciSonucu:
    ...
    istasyon, personel, personel_kullanimda = _operator_baglantilari(db, rol, veri.get("istasyon_id"), veri.get("personel_id"))
    if rol == "Operatör" and (not istasyon or not personel or personel_kullanimda):
        return KullaniciSonucu(hata="Operatör için aktif istasyon ve başka hesaba atanmamış aktif personel seçilmelidir.")
    ...
    kullanici = User(kullanici_adi=kullanici_adi, ad_soyad=veri["ad_soyad"], telefon=veri.get("telefon", ""),
        email=email, rol=rol, istasyon_id=istasyon.id if istasyon else None, personel_id=personel.id if personel else None,
...
def kullanici_guncelle(db: Session, kullanici_id: int, **veri) -> KullaniciSonucu:
    ...
    istasyon, personel, personel_kullanimda = _operator_baglantilari(db, rol, veri.get("istasyon_id"), veri.get("personel_id"), kullanici_id)
    if rol == "Operatör" and (not istasyon or not personel or personel_kullanimda):
        return KullaniciSonucu(kullanici=kullanici, hata="Operatör için aktif istasyon ve başka hesaba atanmamış aktif personel seçilmelidir.")
    ...
    kullanici.istasyon_id, kullanici.personel_id = (istasyon.id, personel.id) if rol == "Operatör" else (None, None)
